...
int main(int argc, char *argv[])
{
	...
	Omega2D::InitGraphics(640, 480, 16);
	Omega2D::InitInput();

	SDL_Surface *playerShip = Omega2D::LoadImage("sd:/apps/wiifighter/source/playerShip.png");
	TTF_Font *hudFont = Omega2D::LoadFont("sd:/apps/wiifighter/source/kenpixel_high_square.ttf", 12);
	SDL_Color whiteColor = {255, 255, 255, 0};
	// SDL_Surface *spaceBackground = Omega2D::LoadImage("sd:/apps/wiifighter/source/spaceBackground.png");
	SDL_Surface *playerLaser = Omega2D::LoadImage("sd:/apps/wiifighter/source/playerLaser.png");
	SDL_Surface *enemyOne = Omega2D::LoadImage("sd:/apps/wiifighter/source/enemyOne.png");
	SDL_Surface *enemyExplosion = Omega2D::LoadImage("sd:/apps/wiifighter/source/enemyExplosion.png");
	SDL_Surface *tiledBackground = Omega2D::LoadImage("sd:/apps/wiifighter/source/tiledBackground.png");
	...
	bgTiles.push_back(Omega2D::Vector2(480, 0));

	float tileSpeed = 0.03f;

	for (int i = 0; i < bgTiles.size(); i++)
	{
		bgTiles.at(i).x = i * 480.0f;
		bgTiles.at(i).y = 0.0f;
	}
	...
	{		
		while (SDL_PollEvent(&event))
		{
			switch (event.type)
			{
				case SDL_JOYBUTTONDOWN:
					if (event.jbutton.button == 3) fireButton = true;
					break;

				case SDL_JOYBUTTONUP:
					if (event.jbutton.button == 3) fireButton = false;
					break;
		...
		if (bgTiles.back().x <= 160.0f)
		{
			bgTiles.push_back(Omega2D::Vector2(bgTiles.back().x + 480.0f, 0.0f));
		...
		playerPos.x = Omega2D::Clamp(playerPos.x, 0.0f, 590.0f);
		playerPos.y = Omega2D::Clamp(playerPos.y, 0.0f, 405.0f);
		...
		if (SDL_GetTicks() > lastEnemySpawn + enemySpawnInterval)
		{
			...
			// spawn an enemy
			Omega2D::Vector2 newEnemy(700, Omega2D::RandomInt(50, 369));
